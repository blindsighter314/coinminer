...
WARNING
THIS VERSION OF COMBOBOX HAS BEEN HEAVILY MODIFIED TO WORK WITH COIN GAME.
THIS WILL NOT WORK AS INTENDED BY THE LIBRARY BY ITSELF
IF YOU WANT TO USE GOOEY IN YOUR PROJECT, DOWNLOAD THE UNMODIFIED VERSION AT https://github.com/blindsighter314/GooeyGUI`
...
function goo.ComboBoxMeta:SetText(str)
	self.text.value = str
end
...
function goo.comboboxMousePressed(x, y, button)
	...
	local topFrame = goo.Frames["storePage"] -- This is a hack to get the store working. This breaks future comboboxes.
			...
			if combobox.isOpen == true then
				for boxNumber,choice in pairs(combobox.choices) do
					if goo.getIntersect(x,y,combobox.x,combobox.y+ (combobox.l*boxNumber),combobox.w,combobox.l)== true then
						...
						store.antiPurchaseThing = true
						store.currentThumbnail = nil
					end
				end
			end

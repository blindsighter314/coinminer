...
function goo.ButtonMeta:CenterText()
	self.text.x = math.floor((self.w / 2) - (goo.getTextWidth(self.text.value, self.font) / 2))
	self.text.y = math.floor((self.l / 2) - (goo.getTextHeight(self.text.value, self.font) / 2))
end
...
function goo.getButton(id)
	...
	for _,frame in pairs(goo.Frames) do
		if frame.children.buttons[id] then return frame.children.buttons[id] end
	end
...
function goo.buttonMousePressed(x, y, b)
	local gotbutton = nil
	for _,button in pairs(goo.Buttons) do
		if goo.getIntersect(x, y, button.x, button.y, button.w, button.l) == true and button.color[4] ~= 0 then
			gotbutton = button
		end
	end

	for _,id in pairs(goo.frameDrawOrder) do
		for _,button in pairs(goo.Frames[id].children.buttons) do
			if goo.getIntersect(x, y, button.x, button.y, button.w, button.l) == true and button.color[4] ~= 0 then
				gotbutton = button
			end
		end
